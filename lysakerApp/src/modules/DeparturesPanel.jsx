...
const ENV_STOP_ID = import.meta.env.VITE_STOPPLACE_ID
const CLIENT = import.meta.env.VITE_ET_CLIENT_NAME
...
export default function DeparturesPanel({
  ...
  // Graceful time drift: oppdater "nå" hvert sekund, re-render sjeldnere (10s)
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (cached) {
          ...
          // Ikke marker cache hvis ingen cache finnes
          onCacheChange?.(false)
        }
        requestAnimationFrame(measure)
